...
echo "Restoring Lambda configuration for: $LAMBDA_NAME"

aws lambda update-function-configuration \
    --function-name "$LAMBDA_NAME" \
    --environment "Variables={GEMINI_API_KEY=$GEMINI_API_KEY,GROQ_API_KEY=$GROQ_API_KEY}" > /dev/null

echo -e "\n\033[1;32m✅ SYSTEM HEALED: Primary Gemini Pipeline Restored.\033[0m\n"
